...
#ATTRIBUTE:PermissionLevel;MANDATORY:True;DEFAULTVALUE:;DESCRIPTION:;CHOICES:Deny All,Deny Write,Full Control,Full Read
$PermissionLevel = Get-Attr -obj $params -name PermissionLevel -failifempty $True -resultobj $result
...
#ATTRIBUTE:ActAsSystemUser;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:
$ActAsSystemUser = Get-Attr -obj $params -name ActAsSystemUser -failifempty $False -resultobj $result
...
If ($PermissionLevel)
{
    If (('Deny All','Deny Write','Full Control','Full Read') -contains $PermissionLevel ) {
    }
    Else
    {
        Fail-Json $result "Option PermissionLevel has invalid value $PermissionLevel. Valid values are 'Deny All','Deny Write','Full Control','Full Read'"
    }
}
...
$DscResourceName = "xSPWebAppPolicy"

...
#ATTRIBUTE:PowerPlan;MANDATORY:True;DEFAULTVALUE:;DESCRIPTION:;CHOICES:Balanced,High performance,Power saver
$PowerPlan = Get-Attr -obj $params -name PowerPlan -failifempty $True -resultobj $result
...
If ($PowerPlan)
{
    If (('Balanced','High performance','Power saver') -contains $PowerPlan ) {
    }
    Else
    {
        Fail-Json $result "Option PowerPlan has invalid value $PowerPlan. Valid values are 'Balanced','High performance','Power saver'"
    }
}
...
$DscResourceName = "cPowerPlan"

$DscModuleName = "cpowerplan"

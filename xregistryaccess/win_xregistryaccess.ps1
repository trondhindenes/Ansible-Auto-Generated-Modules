...
#ATTRIBUTE:AccessControlType;MANDATORY:True;DEFAULTVALUE:;DESCRIPTION:;CHOICES:Allow,Deny
$AccessControlType = Get-Attr -obj $params -name AccessControlType -failifempty $True -resultobj $result
#ATTRIBUTE:Account;MANDATORY:True;DEFAULTVALUE:;DESCRIPTION:;CHOICES:
...
#ATTRIBUTE:RegistryRights;MANDATORY:True;DEFAULTVALUE:;DESCRIPTION:;CHOICES:
$RegistryRights = Get-Attr -obj $params -name RegistryRights -failifempty $True -resultobj $result
#ATTRIBUTE:InheritanceFlags;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:ContainerInherit,None,ObjectInherit
$InheritanceFlags = Get-Attr -obj $params -name InheritanceFlags -failifempty $False -resultobj $result
#ATTRIBUTE:Location;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:ClassesRoot,CurrentConfig,CurrentUser,DynData,LocalMachine,PerformanceData,Users
$Location = Get-Attr -obj $params -name Location -failifempty $False -resultobj $result
#ATTRIBUTE:PropagationFlags;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:InheritOnly,None,NoPropagateInherit
$PropagationFlags = Get-Attr -obj $params -name PropagationFlags -failifempty $False -resultobj $result
...
{
    If (('Allow','Deny') -contains $AccessControlType ) {
    }
    Else
    {
        Fail-Json $result "Option AccessControlType has invalid value $AccessControlType. Valid values are 'Allow','Deny'"
    }
}
...
If ($InheritanceFlags)
{
    If (('ContainerInherit','None','ObjectInherit') -contains $InheritanceFlags ) {
    }
    Else
    {
        Fail-Json $result "Option InheritanceFlags has invalid value $InheritanceFlags. Valid values are 'ContainerInherit','None','ObjectInherit'"
    }
}
...
If ($Location)
{
    If (('ClassesRoot','CurrentConfig','CurrentUser','DynData','LocalMachine','PerformanceData','Users') -contains $Location ) {
    }
    Else
    {
        Fail-Json $result "Option Location has invalid value $Location. Valid values are 'ClassesRoot','CurrentConfig','CurrentUser','DynData','LocalMachine','PerformanceData','Users'"
    }
}
...
If ($PropagationFlags)
{
    If (('InheritOnly','None','NoPropagateInherit') -contains $PropagationFlags ) {
    }
    Else
    {
        Fail-Json $result "Option PropagationFlags has invalid value $PropagationFlags. Valid values are 'InheritOnly','None','NoPropagateInherit'"
    }
}
...
$DscResourceName = "xRegistryAccess"

$DscModuleName = "xregistryaccess"

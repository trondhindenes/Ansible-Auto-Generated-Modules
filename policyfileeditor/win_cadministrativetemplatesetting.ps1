...
If ($PolicyType)
{
    If (('Administrators','Machine','NonAdministrators','User') -contains $PolicyType ) {
    }
    Else
    {
        Fail-Json $result "Option PolicyType has invalid value $PolicyType. Valid values are 'Administrators','Machine','NonAdministrators','User'"
    }
}
...
If ($Type)
{
    If (('Binary','DWord','ExpandString','MultiString','None','QWord','String','Unknown') -contains $Type ) {
    }
    Else
    {
...
$DscResourceName = "cAdministrativeTemplateSetting"

$DscModuleName = "policyfileeditor"

...
#ATTRIBUTE:LoginMode;MANDATORY:True;DEFAULTVALUE:;DESCRIPTION:;CHOICES:Mixed,Windows
$LoginMode = Get-Attr -obj $params -name LoginMode -failifempty $True -resultobj $result
...
If ($LoginMode)
{
    If (('Mixed','Windows') -contains $LoginMode ) {
    }
    Else
    {
        Fail-Json $result "Option LoginMode has invalid value $LoginMode. Valid values are 'Mixed','Windows'"
    }
}
...
$DscResourceName = "xDatabaseServer"

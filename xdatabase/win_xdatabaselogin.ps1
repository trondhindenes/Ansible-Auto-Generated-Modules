...
#ATTRIBUTE:SqlAuthType;MANDATORY:True;DEFAULTVALUE:;DESCRIPTION:;CHOICES:SQL,Windows
$SqlAuthType = Get-Attr -obj $params -name SqlAuthType -failifempty $True -resultobj $result
...
If ($SqlAuthType)
{
    If (('SQL','Windows') -contains $SqlAuthType ) {
    }
    Else
    {
...
if ($SqlConnectionCredential_username)
{
$SqlConnectionCredential_securepassword = $SqlConnectionCredential_password | ConvertTo-SecureString -asPlainText -Force
$SqlConnectionCredential = New-Object System.Management.Automation.PSCredential($SqlConnectionCredential_username,$SqlConnectionCredential_securepassword)
}

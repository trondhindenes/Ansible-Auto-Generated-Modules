...
#ATTRIBUTE:Type;MANDATORY:True;DEFAULTVALUE:;DESCRIPTION:;CHOICES:*,file,ftp,http,https,knownfolder,ldap,news,nntp,oecmd,shell,snews
...
#ATTRIBUTE:Zone;MANDATORY:True;DEFAULTVALUE:;DESCRIPTION:;CHOICES:Internet,LocalIntranet,MyComputer,Restricted,TrustedSite
...
#ATTRIBUTE:Platform;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:All,x64,x86
...
If ($Type)
{
    If (('*','file','ftp','http','https','knownfolder','ldap','news','nntp','oecmd','shell','snews') -contains $Type ) {
    }
    Else
    {
        Fail-Json $result "Option Type has invalid value $Type. Valid values are '*','file','ftp','http','https','knownfolder','ldap','news','nntp','oecmd','shell','snews'"
    }
}
...
If ($Zone)
{
    If (('Internet','LocalIntranet','MyComputer','Restricted','TrustedSite') -contains $Zone ) {
    }
    Else
    {
        Fail-Json $result "Option Zone has invalid value $Zone. Valid values are 'Internet','LocalIntranet','MyComputer','Restricted','TrustedSite'"
    }
}
...
If ($Platform)
{
    If (('All','x64','x86') -contains $Platform ) {
    }
    Else
    {
        Fail-Json $result "Option Platform has invalid value $Platform. Valid values are 'All','x64','x86'"
    }
}
...
$DscResourceName = "ZoneSite"

$DscModuleName = "internetsettings"

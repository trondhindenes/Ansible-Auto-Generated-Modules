...
#ATTRIBUTE:SCSPFAdminCredential_username;MANDATORY:True;DEFAULTVALUE:;DESCRIPTION:;CHOICES:
$SCSPFAdminCredential_username = Get-Attr -obj $params -name SCSPFAdminCredential_username -failifempty $True -resultobj $result
#ATTRIBUTE:SCSPFAdminCredential_password;MANDATORY:True;DEFAULTVALUE:;DESCRIPTION:;CHOICES:
$SCSPFAdminCredential_password = Get-Attr -obj $params -name SCSPFAdminCredential_password -failifempty $True -resultobj $result
#ATTRIBUTE:ServerType;MANDATORY:True;DEFAULTVALUE:;DESCRIPTION:;CHOICES:DPM,None,OM,OMDW,Orchestrator,RDGateway,VMM
$ServerType = Get-Attr -obj $params -name ServerType -failifempty $True -resultobj $result
...
If ($ServerType)
{
    If (('DPM','None','OM','OMDW','Orchestrator','RDGateway','VMM') -contains $ServerType ) {
    }
    Else
    {
        Fail-Json $result "Option ServerType has invalid value $ServerType. Valid values are 'DPM','None','OM','OMDW','Orchestrator','RDGateway','VMM'"
    }
}
...
if ($SCSPFAdminCredential_username)
{
$SCSPFAdminCredential_securepassword = $SCSPFAdminCredential_password | ConvertTo-SecureString -asPlainText -Force
$SCSPFAdminCredential = New-Object System.Management.Automation.PSCredential($SCSPFAdminCredential_username,$SCSPFAdminCredential_securepassword)
}
...
$DscResourceName = "xSCSPFServer"

$DscModuleName = "xscspf"

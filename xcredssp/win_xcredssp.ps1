...
#ATTRIBUTE:Role;MANDATORY:True;DEFAULTVALUE:;DESCRIPTION:;CHOICES:Client,Server
$Role = Get-Attr -obj $params -name Role -failifempty $True -resultobj $result
#ATTRIBUTE:DelegateComputers;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:
$DelegateComputers = Get-Attr -obj $params -name DelegateComputers -failifempty $False -resultobj $result
...
If ($Role)
{
    If (('Client','Server') -contains $Role ) {
    }
    Else
    {
        Fail-Json $result "Option Role has invalid value $Role. Valid values are 'Client','Server'"
    }
}
...
$DscResourceName = "xCredSSP"

$DscModuleName = "xcredssp"

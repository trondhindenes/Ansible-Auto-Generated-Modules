...
#ATTRIBUTE:Transport;MANDATORY:True;DEFAULTVALUE:;DESCRIPTION:;CHOICES:HTTP,HTTPS
$Transport = Get-Attr -obj $params -name Transport -failifempty $True -resultobj $result
...
#ATTRIBUTE:Issuer;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:
$Issuer = Get-Attr -obj $params -name Issuer -failifempty $False -resultobj $result
#ATTRIBUTE:MatchAlternate;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:
$MatchAlternate = Get-Attr -obj $params -name MatchAlternate -failifempty $False -resultobj $result
...
#ATTRIBUTE:SubjectFormat;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:Both,FQDNOnly,NameOnly
$SubjectFormat = Get-Attr -obj $params -name SubjectFormat -failifempty $False -resultobj $result
...
If ($Transport)
{
    If (('HTTP','HTTPS') -contains $Transport ) {
    }
    Else
    {
        Fail-Json $result "Option Transport has invalid value $Transport. Valid values are 'HTTP','HTTPS'"
    }
}
...
If ($SubjectFormat)
{
    If (('Both','FQDNOnly','NameOnly') -contains $SubjectFormat ) {
    }
    Else
    {
        Fail-Json $result "Option SubjectFormat has invalid value $SubjectFormat. Valid values are 'Both','FQDNOnly','NameOnly'"
    }
}
...
$DscResourceName = "cWSManListener"

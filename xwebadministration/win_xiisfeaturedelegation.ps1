...
#ATTRIBUTE:OverrideMode;MANDATORY:True;DEFAULTVALUE:;DESCRIPTION:;CHOICES:Allow,Deny
$OverrideMode = Get-Attr -obj $params -name OverrideMode -failifempty $True -resultobj $result
#ATTRIBUTE:SectionName;MANDATORY:True;DEFAULTVALUE:;DESCRIPTION:;CHOICES:
$SectionName = Get-Attr -obj $params -name SectionName -failifempty $True -resultobj $result
...
If ($OverrideMode)
{
    If (('Allow','Deny') -contains $OverrideMode ) {
    }
    Else
    {
        Fail-Json $result "Option OverrideMode has invalid value $OverrideMode. Valid values are 'Allow','Deny'"
    }
}
...
$DscResourceName = "xIisFeatureDelegation"

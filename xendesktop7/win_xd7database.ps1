...
#ATTRIBUTE:DataStore;MANDATORY:True;DEFAULTVALUE:;DESCRIPTION:;CHOICES:Logging,Monitor,Site
$DataStore = Get-Attr -obj $params -name DataStore -failifempty $True -resultobj $result
...
If ($DataStore)
{
    If (('Logging','Monitor','Site') -contains $DataStore ) {
    }
    Else
    {
        Fail-Json $result "Option DataStore has invalid value $DataStore. Valid values are 'Logging','Monitor','Site'"
    }
}

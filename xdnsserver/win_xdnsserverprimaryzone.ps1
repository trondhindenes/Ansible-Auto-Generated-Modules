...
#ATTRIBUTE:DynamicUpdate;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:None,NonsecureAndSecure
...
#ATTRIBUTE:ZoneFile;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:
$ZoneFile = Get-Attr -obj $params -name ZoneFile -failifempty $False -resultobj $result
...
If ($DynamicUpdate)
{
    If (('None','NonsecureAndSecure') -contains $DynamicUpdate ) {
    }
    Else
    {
        Fail-Json $result "Option DynamicUpdate has invalid value $DynamicUpdate. Valid values are 'None','NonsecureAndSecure'"
    }
}
...
$DscResourceName = "xDnsServerPrimaryZone"

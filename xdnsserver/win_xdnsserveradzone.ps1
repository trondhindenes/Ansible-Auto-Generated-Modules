...
#ATTRIBUTE:ReplicationScope;MANDATORY:True;DEFAULTVALUE:;DESCRIPTION:;CHOICES:Custom,Domain,Forest,Legacy
$ReplicationScope = Get-Attr -obj $params -name ReplicationScope -failifempty $True -resultobj $result
#ATTRIBUTE:ComputerName;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:
$ComputerName = Get-Attr -obj $params -name ComputerName -failifempty $False -resultobj $result
...
#ATTRIBUTE:DirectoryPartitionName;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:
$DirectoryPartitionName = Get-Attr -obj $params -name DirectoryPartitionName -failifempty $False -resultobj $result
#ATTRIBUTE:DynamicUpdate;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:None,NonsecureAndSecure,Secure
$DynamicUpdate = Get-Attr -obj $params -name DynamicUpdate -failifempty $False -resultobj $result
...
If ($ReplicationScope)
{
    If (('Custom','Domain','Forest','Legacy') -contains $ReplicationScope ) {
    }
    Else
    {
        Fail-Json $result "Option ReplicationScope has invalid value $ReplicationScope. Valid values are 'Custom','Domain','Forest','Legacy'"
    }
}
...
If ($DynamicUpdate)
{
    If (('None','NonsecureAndSecure','Secure') -contains $DynamicUpdate ) {
    }
    Else
    {
        Fail-Json $result "Option DynamicUpdate has invalid value $DynamicUpdate. Valid values are 'None','NonsecureAndSecure','Secure'"
    }
}
...
$DscResourceName = "xDnsServerADZone"

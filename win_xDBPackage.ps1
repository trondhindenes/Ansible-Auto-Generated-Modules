...
#ATTRIBUTE:Type;MANDATORY:True;DEFAULTVALUE:;DESCRIPTION:;CHOICES:BACPAC,DACPAC
...
If ($Type)
{
    If (('BACPAC','DACPAC') -contains $Type ) {
    }
    Else
    {
        Fail-Json $result "Option Type has invalid value $Type. Valid values are 'BACPAC','DACPAC'"
    }
}
...
{
$Credentials_securepassword = $Credentials_password | ConvertTo-SecureString -asPlainText -Force
$Credentials = New-Object System.Management.Automation.PSCredential($Credentials_username,$Credentials_securepassword)
}

$DscResourceName = "xDBPackage"

...
#ATTRIBUTE:IPv4Connectivity;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:Disconnected,Internet,LocalNetwork,NoTraffic,Subnet
$IPv4Connectivity = Get-Attr -obj $params -name IPv4Connectivity -failifempty $False -resultobj $result
#ATTRIBUTE:IPv6Connectivity;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:Disconnected,Internet,LocalNetwork,NoTraffic,Subnet
$IPv6Connectivity = Get-Attr -obj $params -name IPv6Connectivity -failifempty $False -resultobj $result
#ATTRIBUTE:NetworkCategory;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:Private,Public
$NetworkCategory = Get-Attr -obj $params -name NetworkCategory -failifempty $False -resultobj $result
...
If ($IPv4Connectivity)
{
    If (('Disconnected','Internet','LocalNetwork','NoTraffic','Subnet') -contains $IPv4Connectivity ) {
    }
    Else
    {
        Fail-Json $result "Option IPv4Connectivity has invalid value $IPv4Connectivity. Valid values are 'Disconnected','Internet','LocalNetwork','NoTraffic','Subnet'"
    }
}
...
If ($IPv6Connectivity)
{
    If (('Disconnected','Internet','LocalNetwork','NoTraffic','Subnet') -contains $IPv6Connectivity ) {
    }
    Else
    {
        Fail-Json $result "Option IPv6Connectivity has invalid value $IPv6Connectivity. Valid values are 'Disconnected','Internet','LocalNetwork','NoTraffic','Subnet'"
    }
}
...
If ($NetworkCategory)
{
    If (('Private','Public') -contains $NetworkCategory ) {
    }
    Else
    {
        Fail-Json $result "Option NetworkCategory has invalid value $NetworkCategory. Valid values are 'Private','Public'"
    }
}
...
$DscResourceName = "xNetConnectionProfile"

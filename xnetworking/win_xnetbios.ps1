...
#ATTRIBUTE:Setting;MANDATORY:True;DEFAULTVALUE:;DESCRIPTION:;CHOICES:Default,Disable,Enable
$Setting = Get-Attr -obj $params -name Setting -failifempty $True -resultobj $result
...
If ($Setting)
{
    If (('Default','Disable','Enable') -contains $Setting ) {
    }
    Else
    {
        Fail-Json $result "Option Setting has invalid value $Setting. Valid values are 'Default','Disable','Enable'"
    }
}

...
#ATTRIBUTE:InstallPolicy;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:Trusted,Untrusted
$InstallPolicy = Get-Attr -obj $params -name InstallPolicy -failifempty $False -resultobj $result
...
#ATTRIBUTE:PublishUri;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:
$PublishUri = Get-Attr -obj $params -name PublishUri -failifempty $False -resultobj $result
...
If ($InstallPolicy)
{
    If (('Trusted','Untrusted') -contains $InstallPolicy ) {
    }
    Else
    {
        Fail-Json $result "Option InstallPolicy has invalid value $InstallPolicy. Valid values are 'Trusted','Untrusted'"
    }
}

...
#ATTRIBUTE:Community;MANDATORY:True;DEFAULTVALUE:;DESCRIPTION:;CHOICES:
$Community = Get-Attr -obj $params -name Community -failifempty $True -resultobj $result
...
#ATTRIBUTE:Right;MANDATORY:True;DEFAULTVALUE:;DESCRIPTION:;CHOICES:None,Notify,ReadCreate,ReadOnly,ReadWrite
$Right = Get-Attr -obj $params -name Right -failifempty $True -resultobj $result
...
If ($Right)
{
    If (('None','Notify','ReadCreate','ReadOnly','ReadWrite') -contains $Right ) {
    }
    Else
    {
        Fail-Json $result "Option Right has invalid value $Right. Valid values are 'None','Notify','ReadCreate','ReadOnly','ReadWrite'"
    }
}
...
$DscResourceName = "cSNMPCommunity"

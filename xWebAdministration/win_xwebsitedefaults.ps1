...
#ATTRIBUTE:ApplyTo;MANDATORY:True;DEFAULTVALUE:;DESCRIPTION:;CHOICES:Machine
$ApplyTo = Get-Attr -obj $params -name ApplyTo -failifempty $True -resultobj $result
#ATTRIBUTE:AllowSubDirConfig;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:false,true
$AllowSubDirConfig = Get-Attr -obj $params -name AllowSubDirConfig -failifempty $False -resultobj $result
#ATTRIBUTE:DefaultApplicationPool;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:
$DefaultApplicationPool = Get-Attr -obj $params -name DefaultApplicationPool -failifempty $False -resultobj $result
...
#ATTRIBUTE:LogFormat;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:Custom,IIS,NCSA,W3C
...
#ATTRIBUTE:TraceLogDirectory;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:
$TraceLogDirectory = Get-Attr -obj $params -name TraceLogDirectory -failifempty $False -resultobj $result
...
If ($ApplyTo)
{
    If (('Machine') -contains $ApplyTo ) {
    }
    Else
    {
        Fail-Json $result "Option ApplyTo has invalid value $ApplyTo. Valid values are 'Machine'"
    }
}


If ($AllowSubDirConfig)
{
    If (('false','true') -contains $AllowSubDirConfig ) {
    }
    Else
    {
        Fail-Json $result "Option AllowSubDirConfig has invalid value $AllowSubDirConfig. Valid values are 'false','true'"
    }
}
...
If ($LogFormat)
{
    If (('Custom','IIS','NCSA','W3C') -contains $LogFormat ) {
    }
    Else
    {
        Fail-Json $result "Option LogFormat has invalid value $LogFormat. Valid values are 'Custom','IIS','NCSA','W3C'"
    }
}
...
$DscResourceName = "xWebSiteDefaults"

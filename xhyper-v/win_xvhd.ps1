...
#ATTRIBUTE:MaximumSizeBytes;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:
$MaximumSizeBytes = Get-Attr -obj $params -name MaximumSizeBytes -failifempty $False -resultobj $result
...
If ($Generation)
{
    If (('Vhd','Vhdx') -contains $Generation ) {
    }
    Else
    {
        Fail-Json $result "Option Generation has invalid value $Generation. Valid values are 'Vhd','Vhdx'"
    }
}

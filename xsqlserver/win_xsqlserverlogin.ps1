...
$LoginCredential_username = Get-Attr -obj $params -name LoginCredential_username -failifempty $False -resultobj $result
#ATTRIBUTE:LoginCredential_password;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:
$LoginCredential_password = Get-Attr -obj $params -name LoginCredential_password -failifempty $False -resultobj $result
#ATTRIBUTE:LoginType;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:SqlLogin,WindowsGroup,WindowsUser
...
If ($LoginType)
{
    If (('SqlLogin','WindowsGroup','WindowsUser') -contains $LoginType ) {
    }
    Else
    {
        Fail-Json $result "Option LoginType has invalid value $LoginType. Valid values are 'SqlLogin','WindowsGroup','WindowsUser'"
    }
}

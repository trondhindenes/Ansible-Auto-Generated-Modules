...
#ATTRIBUTE:ServerRole;MANDATORY:True;DEFAULTVALUE:;DESCRIPTION:;CHOICES:bulkadmin,dbcreator,diskadmin,processadmin,public,securityadmin,serveradmin,setupadmin,sysadmin
$ServerRole = Get-Attr -obj $params -name ServerRole -failifempty $True -resultobj $result
...
If ($ServerRole)
{
    If (('bulkadmin','dbcreator','diskadmin','processadmin','public','securityadmin','serveradmin','setupadmin','sysadmin') -contains $ServerRole ) {
    }
    Else
    {
        Fail-Json $result "Option ServerRole has invalid value $ServerRole. Valid values are 'bulkadmin','dbcreator','diskadmin','processadmin','public','securityadmin','serveradmin','setupadmin','sysadmin'"
    }
}

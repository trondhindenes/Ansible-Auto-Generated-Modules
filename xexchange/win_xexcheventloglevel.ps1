...
#ATTRIBUTE:Level;MANDATORY:True;DEFAULTVALUE:;DESCRIPTION:;CHOICES:Expert,High,Low,Lowest,Medium
$Level = Get-Attr -obj $params -name Level -failifempty $True -resultobj $result
...
If ($Level)
{
    If (('Expert','High','Low','Lowest','Medium') -contains $Level ) {
    }
    Else
    {
        Fail-Json $result "Option Level has invalid value $Level. Valid values are 'Expert','High','Low','Lowest','Medium'"
    }
}
...
$DscResourceName = "xExchEventLogLevel"

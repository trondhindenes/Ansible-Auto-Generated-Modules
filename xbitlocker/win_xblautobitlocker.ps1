...
#ATTRIBUTE:DriveType;MANDATORY:True;DEFAULTVALUE:;DESCRIPTION:;CHOICES:Fixed,Removable
...
#ATTRIBUTE:MinDiskCapacityGB;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:
...
If ($DriveType)
{
    ...
    Else
    {
        Fail-Json $result "Option DriveType has invalid value $DriveType. Valid values are 'Fixed','Removable'"
    }
}
...
If ($PrimaryProtector)
{
    If (('PasswordProtector','RecoveryPasswordProtector','StartupKeyProtector','TpmProtector') -contains $PrimaryProtector ) {
    }
    Else
    {
        Fail-Json $result "Option PrimaryProtector has invalid value $PrimaryProtector. Valid values are 'PasswordProtector','RecoveryPasswordProtector','StartupKeyProtector','TpmProtector'"
    }
}
...
If ($EncryptionMethod)
{
    If (('Aes128','Aes256') -contains $EncryptionMethod ) {
    }
    Else
    {
...
if ($Pin_username)
{
$Pin_securepassword = $Pin_password | ConvertTo-SecureString -asPlainText -Force
$Pin = New-Object System.Management.Automation.PSCredential($Pin_username,$Pin_securepassword)
}
...
$DscResourceName = "xBLAutoBitlocker"

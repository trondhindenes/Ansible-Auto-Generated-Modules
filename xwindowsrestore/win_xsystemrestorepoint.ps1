...
#ATTRIBUTE:RestorePointType;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:APPLICATION_INSTALL,APPLICATION_UNINSTALL,CANCELLED_OPERATION,DEVICE_DRIVER_INSTALL,MODIFY_SETTINGS
$RestorePointType = Get-Attr -obj $params -name RestorePointType -failifempty $False -resultobj $result
...
If ($RestorePointType)
{
    If (('APPLICATION_INSTALL','APPLICATION_UNINSTALL','CANCELLED_OPERATION','DEVICE_DRIVER_INSTALL','MODIFY_SETTINGS') -contains $RestorePointType ) {
    }
    Else
    {
        Fail-Json $result "Option RestorePointType has invalid value $RestorePointType. Valid values are 'APPLICATION_INSTALL','APPLICATION_UNINSTALL','CANCELLED_OPERATION','DEVICE_DRIVER_INSTALL','MODIFY_SETTINGS'"
    }
}
...
$DscResourceName = "xSystemRestorePoint"

$DscModuleName = "xwindowsrestore"

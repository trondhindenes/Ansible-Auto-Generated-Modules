...
#ATTRIBUTE:ManagementServer;MANDATORY:True;DEFAULTVALUE:;DESCRIPTION:;CHOICES:
$ManagementServer = Get-Attr -obj $params -name ManagementServer -failifempty $True -resultobj $result
...
#ATTRIBUTE:WebConsoleAuthorizationMode;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:Mixed,Network
$WebConsoleAuthorizationMode = Get-Attr -obj $params -name WebConsoleAuthorizationMode -failifempty $False -resultobj $result
#ATTRIBUTE:WebConsoleUseSSL;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:
$WebConsoleUseSSL = Get-Attr -obj $params -name WebConsoleUseSSL -failifempty $False -resultobj $result
...
{
    If (('Always','Never','Queued') -contains $EnableErrorReporting ) {
    }
    Else
    {
...
If ($WebConsoleAuthorizationMode)
{
    If (('Mixed','Network') -contains $WebConsoleAuthorizationMode ) {
    }
    Else
    {
        Fail-Json $result "Option WebConsoleAuthorizationMode has invalid value $WebConsoleAuthorizationMode. Valid values are 'Mixed','Network'"
    }
}
...
$DscResourceName = "xSCOMWebConsoleServerSetup"

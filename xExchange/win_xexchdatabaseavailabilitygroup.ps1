...
{
    If (('DagOnly','Off') -contains $DatacenterActivationMode ) {
    }
    Else
    {
...
{
    If (('Disabled','Enabled','InterSubnetOnly','SeedOnly') -contains $NetworkCompression ) {
    }
    Else
    {
        Fail-Json $result "Option NetworkCompression has invalid value $NetworkCompression. Valid values are 'Disabled','Enabled','InterSubnetOnly','SeedOnly'"
    }
}


If ($NetworkEncryption)
{
    If (('Disabled','Enabled','InterSubnetOnly','SeedOnly') -contains $NetworkEncryption ) {
    }
    Else
    {
...
$DscResourceName = "xExchDatabaseAvailabilityGroup"

...
$AdminContentDatabaseName = Get-Attr -obj $params -name AdminContentDatabaseName -failifempty $True -resultobj $result
...
$Passphrase_username = Get-Attr -obj $params -name Passphrase_username -failifempty $True -resultobj $result
#ATTRIBUTE:Passphrase_password;MANDATORY:True;DEFAULTVALUE:;DESCRIPTION:;CHOICES:
$Passphrase_password = Get-Attr -obj $params -name Passphrase_password -failifempty $True -resultobj $result
#ATTRIBUTE:CentralAdministrationAuth;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:Kerberos,NTLM
$CentralAdministrationAuth = Get-Attr -obj $params -name CentralAdministrationAuth -failifempty $False -resultobj $result
...
#ATTRIBUTE:ServerRole;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:Application,Custom,DistributedCache,Search,SingleServer,SingleServerFarm,SpecialLoad,WebFrontEnd
$ServerRole = Get-Attr -obj $params -name ServerRole -failifempty $False -resultobj $result
...
If ($ServerRole)
{
    If (('Application','Custom','DistributedCache','Search','SingleServer','SingleServerFarm','SpecialLoad','WebFrontEnd') -contains $ServerRole ) {
    }
    Else
    {
        Fail-Json $result "Option ServerRole has invalid value $ServerRole. Valid values are 'Application','Custom','DistributedCache','Search','SingleServer','SingleServerFarm','SpecialLoad','WebFrontEnd'"
    }
}

...
#ATTRIBUTE:Reference;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:DnsServer,StaticIp
$Reference = Get-Attr -obj $params -name Reference -failifempty $False -resultobj $result
...
If ($Reference)
{
    If (('DnsServer','StaticIp') -contains $Reference ) {
    }
    Else
    {
        Fail-Json $result "Option Reference has invalid value $Reference. Valid values are 'DnsServer','StaticIp'"
    }
}

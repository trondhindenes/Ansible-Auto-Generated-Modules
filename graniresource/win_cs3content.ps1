...
#ATTRIBUTE:S3BucketName;MANDATORY:True;DEFAULTVALUE:;DESCRIPTION:;CHOICES:
$S3BucketName = Get-Attr -obj $params -name S3BucketName -failifempty $True -resultobj $result
#ATTRIBUTE:CheckSum;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:FileHash,FileName
...
#ATTRIBUTE:Key;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:
$Key = Get-Attr -obj $params -name Key -failifempty $False -resultobj $result
#ATTRIBUTE:PostAction;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:
$PostAction = Get-Attr -obj $params -name PostAction -failifempty $False -resultobj $result
#ATTRIBUTE:PreAction;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:
$PreAction = Get-Attr -obj $params -name PreAction -failifempty $False -resultobj $result
...
If ($CheckSum)
{
    If (('FileHash','FileName') -contains $CheckSum ) {
    }
    Else
    {
        Fail-Json $result "Option CheckSum has invalid value $CheckSum. Valid values are 'FileHash','FileName'"
    }
}

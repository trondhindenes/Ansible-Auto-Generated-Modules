...
#ATTRIBUTE:CertStoreName;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:AddressBook,AuthRoot,CertificateAuthority,Disallowed,My,Root,TrustedPeople,TrustedPublisher
$CertStoreName = Get-Attr -obj $params -name CertStoreName -failifempty $False -resultobj $result
...
#ATTRIBUTE:PfxFilePath;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:
$PfxFilePath = Get-Attr -obj $params -name PfxFilePath -failifempty $False -resultobj $result
...
If ($CertStoreName)
{
    If (('AddressBook','AuthRoot','CertificateAuthority','Disallowed','My','Root','TrustedPeople','TrustedPublisher') -contains $CertStoreName ) {
    }
    Else
    {
        Fail-Json $result "Option CertStoreName has invalid value $CertStoreName. Valid values are 'AddressBook','AuthRoot','CertificateAuthority','Disallowed','My','Root','TrustedPeople','TrustedPublisher'"
    }
}

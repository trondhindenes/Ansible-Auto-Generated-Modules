...
#ATTRIBUTE:EnableLUA;MANDATORY:True;DEFAULTVALUE:;DESCRIPTION:;CHOICES:false,true
$EnableLUA = Get-Attr -obj $params -name EnableLUA -failifempty $True -resultobj $result
...
If ($EnableLUA)
{
    If (('false','true') -contains $EnableLUA ) {
    }
    Else
    {
        Fail-Json $result "Option EnableLUA has invalid value $EnableLUA. Valid values are 'false','true'"
    }
}
...
$DscResourceName = "cEPRSDisableUAC"

$DscModuleName = "ceprsdisableuac"

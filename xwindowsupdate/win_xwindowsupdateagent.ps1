...
#ATTRIBUTE:IsSingleInstance;MANDATORY:True;DEFAULTVALUE:;DESCRIPTION:;CHOICES:Yes
$IsSingleInstance = Get-Attr -obj $params -name IsSingleInstance -failifempty $True -resultobj $result
#ATTRIBUTE:Source;MANDATORY:True;DEFAULTVALUE:;DESCRIPTION:;CHOICES:MicrosoftUpdate,WindowsUpdate,WSUS
$Source = Get-Attr -obj $params -name Source -failifempty $True -resultobj $result
#ATTRIBUTE:Category;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:Important,Optional,Security
$Category = Get-Attr -obj $params -name Category -failifempty $False -resultobj $result
#ATTRIBUTE:Notifications;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:Disabled,ScheduledInstallation
$Notifications = Get-Attr -obj $params -name Notifications -failifempty $False -resultobj $result
...
#ATTRIBUTE:UpdateNow;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:
$UpdateNow = Get-Attr -obj $params -name UpdateNow -failifempty $False -resultobj $result
...
If ($IsSingleInstance)
{
    If (('Yes') -contains $IsSingleInstance ) {
    }
    Else
    {
        Fail-Json $result "Option IsSingleInstance has invalid value $IsSingleInstance. Valid values are 'Yes'"
    }
}
...
If ($Source)
{
    If (('MicrosoftUpdate','WindowsUpdate','WSUS') -contains $Source ) {
    }
    Else
    {
        Fail-Json $result "Option Source has invalid value $Source. Valid values are 'MicrosoftUpdate','WindowsUpdate','WSUS'"
    }
}
...
If ($Category)
{
    If (('Important','Optional','Security') -contains $Category ) {
    }
    Else
    {
        Fail-Json $result "Option Category has invalid value $Category. Valid values are 'Important','Optional','Security'"
    }
}
...
If ($Notifications)
{
    If (('Disabled','ScheduledInstallation') -contains $Notifications ) {
    }
    Else
    {
        Fail-Json $result "Option Notifications has invalid value $Notifications. Valid values are 'Disabled','ScheduledInstallation'"
    }
}
...
$DscResourceName = "xWindowsUpdateAgent"

$DscModuleName = "xwindowsupdate"

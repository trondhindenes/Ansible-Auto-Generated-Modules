...
#ATTRIBUTE:Target;MANDATORY:True;DEFAULTVALUE:;DESCRIPTION:;CHOICES:Membership,Windows
$Target = Get-Attr -obj $params -name Target -failifempty $True -resultobj $result
...
If ($Target)
{
    If (('Membership','Windows') -contains $Target ) {
    }
    Else
    {
        Fail-Json $result "Option Target has invalid value $Target. Valid values are 'Membership','Windows'"
    }
}
...
$DscResourceName = "xAzurePackIdentityProvider"
